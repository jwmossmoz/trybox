...
func commandFlags(name string, spec flagSpec) (*flag.FlagSet, *options) {
	opts := &options{Headless: true, Resources: spec.Resources}
	...
	if spec.Resources {
		fs.IntVar(&opts.CPU, "cpu", 0, "override VM CPU count")
		fs.IntVar(&opts.MemoryMB, "memory-mb", 0, "override VM memory in MiB")
		fs.IntVar(&opts.DiskGB, "disk-gb", 0, "override VM disk size in GiB")
	}
	return fs, opts
}
